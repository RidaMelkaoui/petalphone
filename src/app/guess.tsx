...
export default function GuessScreen() {
  ...
  return (
    <AppScreen keyboard testID="guess-screen" contentStyle={{ gap: spacing.sm }}>
      ...
      <PetalInput accessibilityLabel="Your guess" autoCapitalize="sentences" autoCorrect editable={!timer.expired} maxLength={80} onChangeText={setGuess} placeholder="Your guess" returnKeyType="done" value={guess} />
      <BodyText centered small>Everyone else should look away.</BodyText>
      <PetalButton disabled={!guess.trim() && !timer.expired} onPress={() => { Keyboard.dismiss(); game.submitGuess(guess); router.replace(isLastTurn ? '/reveal' : '/pass'); }}>
